...
namespace NODE_1
{
    ...
    public:
        ...
        // CONSTRUCTOR
        //init_data = value_type() : This notation means “the parameter named init_data has a default argument 
        //that is created by the value_type default constructor.”
        node(const value_type& init_data = value_type(), 
             node* init_link = NULL)
             {
        ...
        /*When the return value of a member function is a pointer to a
            ...
            version that returns a , and an ordinary
    ...
    private:
        value_type data_field; //a piece of data (a double number in our example)
        node* link_field; //link to the next node.
    ...
    be activated by a specific node, and the empty list has no nodes!).*/
